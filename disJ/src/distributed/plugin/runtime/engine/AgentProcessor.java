...
public abstract class AgentProcessor implements IProcessor {
	...
	/*
	 * Logger to log every activities of this process for replay action
	 */
	Logger log;
	...
	/*
	 * A mapping table of every possible state name and value of agent which is
	 * defined in user algorithm
	 */
	 ...
	 * 
	 * @param graph
	 *            A graph model that used by the processor
	 * @param clientRandom
	 *            A client IRandom object the hold algorithm
	 * @param clientAdver
	 *            An adversary Class<AgentControl> object the hold algorithm
	 * @param out
	 *            A URL to a location of log files directory
	 */
	...
	private void setSystemOutConsole() {
		MessageConsole console = SimulatorEngine
				.findConsole(IGraphEditorConstants.DISJ_CONSOLE);
	...
	/*
	 * Write exception message to Eclipse plug-in console then throw
	 * RuntimeException
	 * 
		...
		try {
			...
			for (int i = 0; i < hosts.size(); i++) {
				...
				for (int j = 0; j < numAgent; j++) {
					...
					agentId++;
					// this.systemOut.println("@loadAgent() " +
					// agent.getAgentId());
				}
			}
			...
			for (int i = 0; i < hosts.size(); i++) {
				...
				for (int j = 0; j < list.size(); j++) {
					...
					if (!agent.isStarter()) {
						execTime = this.randomGen
								.nextInt(IConstants.MAX_RANDOM_RANGE);
					}
					// add an event into a queue
					Event e = new AgentEvent(IConstants.EVENT_INITIATE_TYPE,
							eventId, execTime, host.getNodeId(), agent
									.getAgentId(), msg);
	...
	private void processNotify(String nodeId, NotifyType type) {
		...
		for (List<NotifyType> tmp = null; its.hasNext();) {
			...
			if (tmp.contains(type)) {
				...
				IMessage msg = new Message(IConstants.MESSAGE_EVENT_NOTIFY,
						type);
	...
	private void processMove(String fromNodeId, String toPort, String agentId) {
		...
		try {
			...
			if (!ports.contains(toPort)) {
				// Notify to user
				this.systemOut
						.println("@processMove() There is no outgoing port "
								+ toPort + " for node " + fromNodeId);
				return;
			}
			...
			int eventId = this.getNextId();
			IMessage msg = new Message(IConstants.MESSAGE_EVENT_ARRIVE_AT, link
					.getEdgeId());
	...
	private void processAlarmClock(String nodeId, String agentId, int delay) {
		try {
			...
			IMessage msg = new Message(IConstants.MESSAGE_EVENT_ARRIVE_AT,
					agentId);

			// create and add event into a queue
			Event e = new AgentEvent(IConstants.EVENT_ALARM_RING_TYPE, eventId,
					execTime, nodeId, agentId, msg);
	...
	private void invokeInit(AgentEvent event) {
		...
		if (!node.isAlive() || !agent.isAlive()) {
			...
			this.log
					.logAgent(logTag.AGENT_INIT, agent.getAgentId() + "", value);
	...
	private void invokeAlarmRing(AgentEvent event) {
		...
		if (!agent.isAlive() || !node.isAlive()) {
			...
			if (agent.hasInitExec()) {
				...
				String value = node.getNodeId();
				this.log
						.logAgent(logTag.AGENT_AWAKE, agent.getAgentId(), value);
	...
	private void invokeArrival(AgentEvent event) throws DisJException {
		...
		// this.systemOut.println("@invokeArrival() Agent: " +
		// agent.getAgentId()
	...
	private void invokeNotify(AgentEvent event) {
		...
		if (!agent.isAlive() || !node.isAlive()) {
			...
			if (agent.hasInitExec()) {
				...
				this.log.logAgent(logTag.AGENT_NOTIFIED, agent.getAgentId(),
						value);
	 ...
	 *            An ID of a link
	 * @param nodeId
	 *            An ID of a destination node
	 * @return a list of agents with expected arrival time if exist, otherwise
