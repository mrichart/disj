...
public abstract class Entity implements ICommunicable {
     ...
     * 
     * @param state A state that need to be set
     */
    ...
    /**
     * Set an internal clock to be alarmed after a give time unit
     * If time is less than or equal to zero, it will ignore
     * 
        ...
        try {
            if (time > 0){
            	this.processor.internalNotify(this.nodeOwner.getNodeId(),
                        new Message(IConstants.SET_ALARM_CLOCK, new Integer(time)));
            }        
        } catch (DisJException e) {
            e.printStackTrace();
    ...
    /**
     * Block and unblock any incoming message through a given port label,
     * state of block is true and unblock is false.
     * 
	 ...
	 * 
	 * @param portLabel An out going port label
	 ...
	 * If a port name does not exist, there will be no message sending.
	 * 
	 * @param portLabels A list of out going port labels
	 * @param message A message that need to be sent
	 */
    ...
    /**
	 * Send a message to all out going ports, execept a port that it just received.
	 * 
	 ...
	 * ports execept a port that it just received.
	 * 
	 * 
	 * NOTE: This will work only for Bidirectional link. For Uni-directional
	 * link, it will work only if a lable of incomming port is the same as 
    ...
    /**
     * Get a current simulation time unit, a time value starts
     * from zero when the process is start running
     * 
     ...
     * 
     * A message casual time is a sum of successive messages that
     * has been caused by another message.<br>
     * 
     * Example<br>
     * <code>
     ...
     * </code>
     * <p>In this example msgA causes the creation of msgB, and msgB causes
     * the creation of msgC and msgD, and msgD casue msgF and so on, 
     * while msgE has no cause to creation of any message.</p>
     * 
     * <p>The current message casual time when msgG is created is 4 becasue
     * there are 4 hops from msgA to msgG from this illustration</p>
     * 
     * 
