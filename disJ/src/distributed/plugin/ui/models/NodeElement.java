...
public class NodeElement extends AdapterElement {
    ...
    /**
     * Set all states of this node back to where is was,
     * when method <code>copyNode</code> is called last.
     */
...
//                + this.location);
    ...
    public void setLocation(Point location) {
        ...
        firePropertyChange(IConstants.PROPERTY_CHANGE_LOCATION, null,
                location);
    }
    ...
    public void resetPropertyValue(Object propName){
        ...
        if (propName.equals(PROPERTY_NAME)) {
            ...
            this.node.setNumMsgRecv(this.orgNode.getNumMsgRecv());
            return;
            
        } else if (propName.equals(PROPERTY_MSG_SENT)) {
            this.node.setNumMsgSend(this.orgNode.getNumMsgSend());
            return;
            
        } else if (propName.equals(PROPERTY_STATES)) {
            		this.node.setStateList(this.orgNode.getStateList());
            return;         
        }
    ...
    public void disconnectInLink(LinkElement link) {
        try {
            //System.out.println(this.node.getNodeId() + "@NodeElement.disconnectInLink() " + link);
            this.node.removeEdge(link.getEdge());
            this.sources.remove(link);
            fireStructureChange(IConstants.PROPERTY_CHANGE_INPUT, link);
        } catch (DisJException ignore) {
            System.err.println("[NodeElement].disconnectInLink() " + this.node.getNodeId() + " : "+ ignore);
        }
    }
    ...
    public void disconnectOutLink(LinkElement link) {
        try{
        //System.out.println(this.node.getNodeId() +  "@NodeElement.disconnectOutLink() " + link);
        this.node.removeEdge(link.getEdge());
        this.targets.remove(link);
        this.fireStructureChange(IConstants.PROPERTY_CHANGE_OUTPUT, link);
        }catch(DisJException ignore){
            System.err.println("@NodeElement.disconnectOutLink() " + this.node.getNodeId() + " : " +ignore);
        }
    }
    ...
    public String toString() {
        return "<NodeElement>Name: " + this.node.getName() + " at: " + getLocation();
    }
